...
class PrometheusClient:
    ...
    def pre_check_deployment_readiness_shortfall(
        self,
        *,
        namespace: str,
        deployment: str,
        ...
        for attempt in range(1, attempts + 1):
            ...
            should_execute = ready_count < float(min_ready_count)
            if should_execute:
                break
            if attempt < attempts:
        ...
        return {
            "status": "ready_to_execute" if should_execute else "not_firing",
            "namespace": namespace,
    ...
    def post_check_deployment_readiness_target(
        self,
        *,
        namespace: str,
        deployment: str,
        ...
        for attempt in range(1, attempts + 1):
            ready_count = self._query_value(ready_query) or 0.0
            recovered = ready_count >= float(min_ready_count)
            if recovered:
                break
            if attempt < attempts:
