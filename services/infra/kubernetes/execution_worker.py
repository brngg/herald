...
def _build_execution_tools(kubernetes: KubernetesClient) -> dict[ExecutionToolName, ExecutionTool]:
    return {
        ...
        "get_networkchaos": ExecutionTool(
            name="get_networkchaos",
            description="Read the current Chaos Mesh NetworkChaos object for the approved scenario.",
            callable=kubernetes.get_networkchaos,
            mutation=False,
        ),
        ...
        "scale_deployment": ExecutionTool(
        ...
        "delete_networkchaos": ExecutionTool(
            name="delete_networkchaos",
            description="Delete the approved Chaos Mesh NetworkChaos object.",
            callable=kubernetes.delete_networkchaos,
            mutation=True,
        ),
    }
...
def _default_worker_command_builder(_: ExecutionDispatch) -> Sequence[str]:
    return [sys.executable, "-m", "services.infra.kubernetes.execution_worker"]
...
def _merged_env(extra_env: Mapping[str, str] | None) -> dict[str, str]:
    ...
    if repo_root not in path_parts:
        env["PYTHONPATH"] = os.pathsep.join([repo_root, *path_parts]) if path_parts else repo_root
    if extra_env:
